...
from commonroad_reach.continuous.continuous_reachability_analysis import ContinuousReachabilityAnalysis
...
def test_generate_tuples_vertices_initial(config: Configuration):
    (
        list_vertices_p_lon,
        list_vertices_p_lat,
    ) = reachset_operation.generate_tuples_vertices_polygons_initial(config)
    ...
    tuple_coords_expected_p_lon = (p_lon - dp, v_x - dv, p_lon + dp, v_x + dv)
...
def test_create_bounding_polygon_returns_correct_vertices():
    polygon = reachset_operation.create_bounding_polygon(dt=2, a_min=-5, a_max=10)
...
def test_create_zero_state_polygon_returns_correct_vertices():
    polygon = reachset_operation.create_zero_state_polygon(dt=2, a_min=-2.0, a_max=2.0)
...
def test_propagate_polygon(config: Configuration):
    ...
    polygon_propagated = reachset_operation.propagate_polygon(
        polygon_lon,
    ...
    assert set(polygon_propagated.vertices) == set(list_vertices_exprected)
...
def test_compute_minimum_positions_of_polygons(
        list_polygons_lon: List[ReachPolygon],
    ...
    p_lon_min, p_lat_min = reachset_operation.compute_minimum_positions_of_rectangles(
        list_polygons_projected
    )
...
@pytest.mark.parametrize(
    "size_grid, tuple_coords_expected",
    [(0.5, (0, 0, 22, 17)), (0.2, (0, 0, 54, 42))],
)
def test_create_discretized_position_rectangles(
        list_polygons_lon: List[ReachPolygon],
        list_polygons_lat: List[ReachPolygon],
        size_grid,
    ...
    list_rectangles_discretized = reachset_operation.discretize_rectangles(
        list_polygons_projected, (p_lon_min, p_lat_min), size_grid
    )
...
@pytest.mark.parametrize(
    "size_grid, tuple_coords_expected",
    [(0.5, (3.5, 4.5, 11, 13.5)), (0.2, (3.2, 3.6, 6.2, 7.2))],
)
def test_create_undiscretized_position_rectangles(
        list_rectangles_discritized,
        size_grid,
        tuple_coords_expected,
):
    p_lon_min = 3
    ...
    list_rectangles_undiscritized = reachset_operation.undiscretized_rectangles(
        list_rectangles_discritized, (p_lon_min, p_lat_min), size_grid
    )
...
def test_create_base_set_from_position_region():
    ...
    list_polygons_lon = [
        ReachPolygon.from_rectangle_vertices(-5, 10, 5, 15),
        ReachPolygon.from_rectangle_vertices(5, 0, 15, 20),
    ]

    list_polygons_lat = [
        ReachPolygon.from_rectangle_vertices(-3, -5, 3, 5),
        ReachPolygon.from_rectangle_vertices(3, 0, 13, 12),
    ]
    list_base_sets = [
        ReachNode(polygon_lon, polygon_lat)
    ...
    base_set_adapted = reachset_operation.adapt_base_set_to_drivable_area(
        rectangle_drivable_area,
        list_base_sets,
