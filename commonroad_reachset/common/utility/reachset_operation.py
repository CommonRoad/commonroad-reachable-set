...
def create_zero_state_polygon(dt: float, a_min: float, a_max: float) -> ReachPolygon:
    ...
    Steps:
        ...
        2. compute coefficients of halfspaces and intersect them with the
           bounding polygon. We use three halfspaces to approximate the upper
           bound of the polygon, this applies to the lower bound as well. A
           halfspace can be specified given a switching time (gamma).
    Args:
    ...
    Returns:
        ReachPolygon: Zero-state polygon
    """
    # Step 1
...
def create_bounding_polygon(dt: float, a_min: float, a_max: float) -> ReachPolygon:
    """Returns a polygon that has the absolute min/max reachable position
    and velocity as its bounds.
...
def compute_halfspace_coefficients(dt: float, a_min: float, a_max: float, gamma: float) -> Tuple:
    ...
    Each halfspace has the form of ax + bv <= c. In the x-v plane, the x and
    v values are both dependent on gamma (switching time, see Söntges T-ITS 2018,
    Sec. III.A). Given a gamma, the maximum and minimum reachable x and v can be
    computed, respectively. The halfspace crossing the point x_ and v_ can be
    obtained through:
    ...
    Returns:
        tuples of coefficients of halfspaces corresponding to upper and lower
        bounds
    """
    """
        ...
        """
    x_upper = a_max * dt ** 2 * (0.5 - gamma + 0.5 * gamma ** 2) + a_min * dt ** 2 * (
            gamma - 0.5 * gamma ** 2
    )
    ...
    x_lower = a_min * dt ** 2 * (0.5 - gamma + 0.5 * gamma ** 2) + a_max * dt ** 2 * (
            gamma - 0.5 * gamma ** 2
    )
...
def propagate_polygon(polygon: ReachPolygon, polygon_zero_state: ReachPolygon, dt: float,
    ...
    Steps:
        ...
        3. compute the minkowski sum of zero-input and zero-state responses
    ...
    Returns:
        ReachPolygon: propagated polygon
    """
...
def create_repartitioned_rectangles(list_rectangles: List[ReachPolygon], size_grid: float) -> List[ReachPolygon]:
    ...
    Returns:
        List[ReachPolygon]: Final list of repartitioned rectangles
    """
...
def discretize_rectangles(list_rectangles: List[ReachPolygon], tuple_p_min_rectangles: Tuple[float, float],
    ...
    size_grid = int(size_grid * 10000)
    p_lon_min_rectangles = int(p_lon_min_rectangles * 10000)
    p_lat_min_rectangles = int(p_lat_min_rectangles * 10000)
    for rectangle in list_rectangles:
        p_lon_min = floor((int(rectangle.p_lon_min * 10000) - p_lon_min_rectangles) / size_grid)
        p_lat_min = floor((int(rectangle.p_lat_min * 10000) - p_lat_min_rectangles) / size_grid)
        p_lon_max = ceil((int(rectangle.p_lon_max * 10000) - p_lon_min_rectangles) / size_grid)
        p_lat_max = ceil((int(rectangle.p_lat_max * 10000) - p_lat_min_rectangles) / size_grid)
...
def repartition_rectangles(list_rectangles: List[ReachPolygon]) -> List[ReachPolygon]:
    ...
    Steps:
        1. Obtain a list of vertical segments representing the contour of the
           union of the rectangles.
        2. Create repartitioned rectangles from the list of vertical segments
           using sweep line algorithm.
        3. Merge adjacent rectangles with same lateral coordinates.
    """
    # Step 1
...
def check_collision_and_split_rectangles(collision_checker: CollisionChecker, time_step: int,
                                         list_rectangles: List[ReachPolygon], radius_terminal_split: float) -> List[
    ReachPolygon]:
    ...
    Returns:
        List[ReachPolygon]: list of collision-free rectangles
    """
...
def create_collision_free_rectangles(time_step: int, collision_checker: CollisionChecker, rectangle: ReachPolygon,
    ...
    If a collision happens between a rectangle and other object, and that the
    diagonal of the rectangle is greater than the terminal radius, it is split
    into two new rectangles in whichever edge (lon/lat) that is longer.
...
def adapt_base_sets_to_drivable_area(drivable_area: List[ReachPolygon],
    ...
    Returns:
        List[ReachBaseSet]: list of adapted base sets
    """
...
def adapt_base_set_to_drivable_area(rectangle_drivable_area: ReachPolygon,
    ...
    Iterate through base sets that are adjacent to the rectangle of drivable
    area under examination (overlaps in position domain), and cut the base
    sets down with position constraints from the rectangle. Non-empty
    intersected lon/lat polygons imply that this is a valid base set and are
    considered as a parent of the rectangle (reachable from the node from
    which the base set is propagated).
    ...
    Returns:
        ReachBaseSet: base set adapted to the drivable area
    """
...
def create_nodes_of_reachable_set(time_step: int, list_base_sets_adapted: List[ReachNode]):
    ...
    Each adapted base set is converted into a reachable set node. Also, the
    parent-child relationship with the nodes of the last time step is updated.
    """
...
def create_nodes_of_semantic_reachable_set(time_step: int, list_base_sets_adapted: List[ReachNode], propositions):
    """Creates list of reachable set nodes of the current time step.
    ...
    for base_set in list_base_sets_adapted:
        node_child = ReachNode(base_set.polygon_lon, base_set.polygon_lat, time_step)
        node_child.set_propositions_persistent = copy.deepcopy(propositions)

        # update parent-child relationship
        for node_parent in base_set.source_propagation:
            node_child.add_parent_node(node_parent)
            node_parent.add_child_node(node_child)
