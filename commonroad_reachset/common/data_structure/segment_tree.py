...
class TreeNodeStatus(Enum):
    ...
    ACTIVE: segment fully activated (from low to high)
...
class CounterTreeNode:
    ...
    def update_status(self):
        ...
        if self.count or self.all_children_are_active():
            self.status = TreeNodeStatus.ACTIVE

        elif self.all_children_are_non_active():
            self.status = TreeNodeStatus.NONACTIVE
...
class CounterSegmentTree:
    ...
    def add_node_interval_to_stack(low: int, high: int, stack: List):
        ...
        The intervals are merged, example:
            stack = []
...
class ToggleTreeNode:
    ...
    def update_status(self):
        """Updates the status of the node."""
        if self.status == TreeNodeStatus.ACTIVE or self.all_children_are_active():
            self.status = TreeNodeStatus.ACTIVE
