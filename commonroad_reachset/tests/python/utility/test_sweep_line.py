...
def test_sort_events():
    list_events = [
        SweepLine.Event(SweepLine.EventType.EXIT, 5, 5, 10),
    ...
    assert (
        list_events[0].p_lon == 3
        and list_events[1].p_lon == 5
        and list_events[2].type == SweepLine.EventType.ENTER
        and list_events[3].p_lat_low == 3
    )


def test_create_event_list():
    list_rectangles = [
        ReachPolygon([(1, 1), (3, 1), (3, 3), (1, 3)]),
        ReachPolygon([(2, 2), (4, 2), (4, 4), (2, 4)]),
    ]
    list_events_expected = [
...
def test_obtain_vertical_segments_from_list_of_rectangles_returns_correct_segments(
    ...
    function_sort = lambda x: x.bounds[0]
...
def test_create_partitioned_rectangles_from_vertical_segments():
    ...
    list_tuples_vertices_rectangles_expected = [
        (1.0, 3.0, 4.0, 7.0),
        (4.0, 0.0, 5.0, 10.0),
        (5.0, 6.0, 7.0, 10.0),
        (5.0, 0.0, 7.0, 4.0),
    ...
    list_rectangles_partitioned = SweepLine.create_rectangles_from_vertical_segments(
...
def test_merge_rectangles_with_same_lateral_coordinates():
    dict_p_lon_to_list_rectangles = {
        0: [ReachPolygon.from_rectangle_vertices(0, 0, 1, 1)],
    ...
    list_rectangles_merged = SweepLine.merge_rectangles_with_same_lateral_coordinates(
        dict_p_lon_to_list_rectangles
    )
