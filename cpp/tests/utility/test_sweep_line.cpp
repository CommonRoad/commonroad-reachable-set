...
TEST_CASE("compute extremum lateral positions of rectangles") {
    auto vec_rectangles = {std::make_shared<ReachPolygon>(1, 1, 3, 3),
                           std::make_shared<ReachPolygon>(2, 2, 4, 4)};
...
TEST_CASE("create event list") {
    ...
    std::vector<SweepLine::Event> vec_events_unwrapped;
...
TEST_CASE("sort events") {
    std::vector<EventPtr> vec_events = {
            std::make_shared<SweepLine::Event>(SweepLine::EventType::EXIT, 5, 5, 10),
            std::make_shared<SweepLine::Event>(SweepLine::EventType::EXIT, 3, 2, 10),
            std::make_shared<SweepLine::Event>(SweepLine::EventType::EXIT, 10, 2, 10),
            std::make_shared<SweepLine::Event>(SweepLine::EventType::ENTER, 10, 3, 10),
            std::make_shared<SweepLine::Event>(SweepLine::EventType::ENTER, 10, 2, 10)
    };
...
TEST_CASE("obtain vertical segments from rectangles returns correct segments") {
    SUBCASE("test case 1") {
        ...
        std::vector<ReachLine> vec_segments_unwrapped;
    ...
    SUBCASE("test case 2") {
        auto vec_rectangles = {std::make_shared<ReachPolygon>(4, 0, 8, 4),
                               std::make_shared<ReachPolygon>(4, 6, 8, 10),
                               std::make_shared<ReachPolygon>(1, 3, 5, 7),
                               std::make_shared<ReachPolygon>(7, 3, 11, 7)};
...
TEST_CASE("merge rectangles with same lateral coordinates") {
    ...
    map_p_lon_to_vec_rectangles[0].emplace_back(std::make_shared<ReachPolygon>(0, 0, 1, 1));
    map_p_lon_to_vec_rectangles[1].emplace_back(std::make_shared<ReachPolygon>(1, 0, 2, 1));
    map_p_lon_to_vec_rectangles[2].emplace_back(std::make_shared<ReachPolygon>(2, 0, 3, 1));
    map_p_lon_to_vec_rectangles[3].emplace_back(std::make_shared<ReachPolygon>(3, 0, 4, 1));
    map_p_lon_to_vec_rectangles[4].emplace_back(std::make_shared<ReachPolygon>(4, 0, 6, 1));
    ...
    CHECK_EQ(vec_rectangles_merged[0]->bounding_box(), std::make_tuple(0, 0, 6, 1));
}
...
TEST_CASE("merge rectangles with same lateral coordinates disconnected") {
    ...
    CHECK_EQ(vec_rectangles_merged[0]->bounding_box(), std::make_tuple(0, 0, 1, 1));
    CHECK_EQ(vec_rectangles_merged[1]->bounding_box(), std::make_tuple(2, 0, 3, 1));
}
...
TEST_CASE("create partitioned rectangles from vertical segments") {
    ...
    std::vector<std::tuple<double, double, double, double>> vec_bounds_rectangles_expected = {{1.0, 3.0, 4.0,  7.0},
                                                                                    {4.0, 0.0, 5.0,  10.0},
                                                                                    {5.0, 6.0, 7.0,  10.0},
                                                                                    {5.0, 0.0, 7.0,  4.0},
                                                                                    {7.0, 0.0, 8.0,  10.0},
