...
public:
    ...
    /// Return the object's current reference counter
...
public:
    ...
    /** \rst
        Manually increase the reference counter of the Python object. Usually, it is
    ...
    /** \rst
        Manually decrease the reference counter of the Python object. Usually, it is
        preferable to use the `object` class which derives from `handle` and calls
        this function automatically. Returns a reference to itself.
    \endrst */
...
/** \rst
    ...
    optionally increases the object's reference counter upon construction, and it
    *always* decreases the reference counter when the `object` instance goes out of
...
public:
    ...
    /// Copy constructor; always increases the reference counter
    object(const object &o) : handle(o) { inc_ref(); }
    /// Move constructor; steals the object from ``other`` and preserves its reference counter
    ...
    /** \rst
        ...
        object's reference counter. The function returns a raw handle to the original
        Python object.
    \endrst */
    handle release() {
      PyObject *tmp = m_ptr;
